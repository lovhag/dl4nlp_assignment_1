...
class MyAttentionModelWithPoolingBatchNorm(nn.Module):
    """My Attention model, based on the Transformer encoder."""

    def __init__(self, vocab_size, embedding_dim, max_seq_len, num_heads, dim_feedforward, num_layers, dropout=0.5):
        super(MyAttentionModelWithPoolingBatchNorm, self).__init__()
        try:
            from torch.nn import TransformerEncoder, TransformerEncoderLayer
        except:
            raise ImportError('TransformerEncoder module does not exist in PyTorch 1.1 or lower.')
        ...
        self.max_seq_len = max_seq_len
        self.decoder = nn.Linear(self.embedding_dim, 1)
        self.batch_normer = torch.nn.BatchNorm1d(self.max_seq_len)
        self.decoder_act = nn.Sigmoid()
    ...
    def forward(self, src, word_position_mask, has_mask=True, padding_mask=None):
        ...
        ##output = torch.add(output, skip_src)
        #print(f"Added output shape: {output.shape}")
        output = self.batch_normer(output.transpose(0,1))
        #print(f"Batch normed output shape: {output.shape}")
        output = self.pooler(output.transpose(1, 2)).squeeze(2)
        #print(f"Pooled output shape: {output.shape}")
        output = self.decoder(output)
        #print(f"Decoder output shape: {output.shape}")
        return self.decoder_act(output)
    
class MyAttentionModelWithPoolingBatchNormSkip(nn.Module):
    """My Attention model, based on the Transformer encoder."""

    def __init__(self, vocab_size, embedding_dim, max_seq_len, num_heads, dim_feedforward, num_layers, dropout=0.5):
        super(MyAttentionModelWithPoolingBatchNormSkip, self).__init__()
        try:
            from torch.nn import TransformerEncoder, TransformerEncoderLayer
        except:
            raise ImportError('TransformerEncoder module does not exist in PyTorch 1.1 or lower.')
...
class MyAttentionModelWithPoolingBatchNormConcat(nn.Module):
    """My Attention model, based on the Transformer encoder."""

    def __init__(self, vocab_size, embedding_dim, max_seq_len, num_heads, dim_feedforward, num_layers, dropout=0.5):
        super(MyAttentionModelWithPoolingBatchNormConcat, self).__init__()
        try:
            from torch.nn import TransformerEncoder, TransformerEncoderLayer
        except:
            raise ImportError('TransformerEncoder module does not exist in PyTorch 1.1 or lower.')
        ...
        self.embedding_dim = embedding_dim
        self.max_seq_len = max_seq_len
        self.decoder = nn.Linear(self.embedding_dim*2, 1)
        self.batch_normer = torch.nn.BatchNorm1d(self.max_seq_len)
        self.decoder_act = nn.Sigmoid()
    ...
    def forward(self, src, word_position_mask, has_mask=True, padding_mask=None):
        ...
        output = self.pooler(output.transpose(1, 2)).squeeze(2)
        #print(f"Pooled output shape: {output.shape}")
        output = torch.cat((output, skip_src), dim=1)
        output = self.decoder(output)
        #print(f"Decoder output shape: {output.shape}")
        return self.decoder_act(output)